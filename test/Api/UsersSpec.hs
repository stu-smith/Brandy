...
spec = do
    ...
    describe "get all users" $ do
        ...
        it "should give empty list" $
            runTest $ \app -> do
                users <- (jsonBody <$> app `get` apiBase) :: IO [WithId PublicUserSummary]
    ...
    describe "get single user" $ do
        ...
        it "should give 200 for get user" $
            runTest $ \app -> do
                ...
                inserted <- jsonBody <$> (app `post` apiBase) insertBody :: IO (WithId PrivateUser)
                let uid = getId inserted
                status <- simpleStatus <$> app `get` (apiBase +/+ uid)
                status `shouldBe` ok200
    ...
    describe "update single user" $ do
        ...
        it "should give 400 for missing displayName" $
            runTest $ \app -> do
                ...
                status <- simpleStatus <$> (app `put` (apiBase +/+ uid)) updateBody
                status `shouldBe` badRequest400
    ...
    describe "delete single user" $ do
        ...
        it "should give 204 for bad key" $
            runTest $ \app -> do
                status <- simpleStatus <$> app `delete` (apiBase +/+ "bad-key")
                status `shouldBe` noContent204

        it "should give 204 for missing key" $
            runTest $ \app -> do
                status <- simpleStatus <$> app `delete` (apiBase +/+ "123")
                status `shouldBe` noContent204
        ...
        it "should give 204 for successful delete user" $
            runTest $ \app -> do
                ...
                inserted <- jsonBody <$> (app `post` apiBase) insertBody :: IO (WithId PrivateUser)
                let uid = getId inserted
                status <- simpleStatus <$> app `delete` (apiBase +/+ uid)
                status `shouldBe` noContent204
        ...
        it "should actually delete the item" $
            runTest $ \app -> do
                ...
                inserted <- jsonBody <$> (app `post` apiBase) insertBody :: IO (WithId PrivateUser)
                let uid = getId inserted
                get1status <- simpleStatus <$> app `get` (apiBase +/+ uid)
                get1status `shouldBe` ok200
                status <- simpleStatus <$> app `delete` (apiBase +/+ uid)
                status `shouldBe` noContent204
                get2status <- simpleStatus <$> app `get` (apiBase +/+ uid)
